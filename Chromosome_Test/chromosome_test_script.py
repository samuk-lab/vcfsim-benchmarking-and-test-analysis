...
def run_simulations(starting_seed, ending_seed):
    ...
    for i in range(starting_seed, ending_seed):
        vcfsim_prompt = f'vcfsim --seed {i} --percent_missing_sites 0 --percent_missing_genotypes 0 --output_file mychrom.vcf --sample_size 10 --chromosome_file chrom.txt'
        bgzip_prompt = 'bgzip -f mychrom.vcf'
        tabix_prompt = 'tabix -f mychrom.vcf.gz'
        pixy_prompt = 'pixy --stats pi --vcf mychrom.vcf.gz --populations populations.txt --include_multiallelic_snps --window_size 10000 --n_cores 2 --output_folder . --output_prefix pixy'
