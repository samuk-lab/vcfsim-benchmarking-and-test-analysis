...
pixy_files <- c("/Users/paimongoulart/VCFSIMPAPER/VCFSim_Tests/Final_VCFSim_Test_Data/Chromosome_Test/final_pixy_pi.txt")
...
exp_pi <- 4*1e6*1e-9
exp_pi_haploid <- 4*5e5*1e-9  # New line added
...
plot_2c <- result_df %>%
  ...
  geom_jitter(width = 0.05, size = 0.075) + 
  stat_summary(fun = mean, geom = "errorbar", aes(ymax = ..y.., ymin = ..y..),
               width = 0.25, size = 2) +
  geom_hline(yintercept = exp_pi) +
  geom_hline(yintercept = exp_pi_haploid, linetype = "dashed") +
  xlab("Chromosome") +
