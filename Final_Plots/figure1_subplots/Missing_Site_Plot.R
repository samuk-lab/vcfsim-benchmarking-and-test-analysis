...
# Read the CSV file
missing <- read.csv('data/missing_sites.csv')
...
# Plot using ggplot2 and assign it to a variable (Figure 1D)
plot_d <- ggplot(missing, aes(x = Percent.Missing, y = Missing.Data)) +
  geom_abline(slope = 1, intercept = 0) +
  geom_point(size = 0.5, alpha = 0.75)+
  labs(x = 'Specified percent missing sites',
       y = 'Measured percent missing sites ') +
